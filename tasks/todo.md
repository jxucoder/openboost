...
## Phase 2 — 场景二:研究可魔改性 ✅ 全部完成(2026-07-20)

custom loss 路径是真优势(文档准确、CPU/GPU 都通)。其余审计确认的差距:

- [x] **注册 API 三件套**:`register_loss` / `register_distribution` / `register_growth_strategy`,
...
- [x] **扩展 cookbook**:每个扩展点一个可复制、被测试覆盖的完整示例(自定义损失/分布/生长策略)
...
- [x] **设备端自定义损失 + 真实 loss_value 回调**:当前每轮 3 次 H2D/D2H 往返;
...
## Phase 3 — 场景三/四:LinearLeaf 与 GAM ✅ 全部完成(2026-07-20)
...
- [x] eval_set/早停/callbacks;sklearn wrapper 停止静默吞 kwargs — `_models/_sklearn.py:960`
- [x] 修文档:`ridge_lambda` 参数不存在(实为 reg_lambda_linear),"继承全部 GradientBoosting 参数"不实
      — `docs/user-guide/models/linear-leaf.md:29`

### OpenBoostGAM
- [x] **成对交互项(GA2M/FAST 风格)**:EBM 有、我们没有,监管场景(年龄×收入)是刚需;
      这是 GAM 最大能力缺口 — `_models/_gam.py:29`
- [x] **相邻 bin 平滑 + 可选单调形状约束**:当前形状函数是逐 bin 原始 Newton 步,稀疏 bin 噪声大;
...
- [x] eval_set/早停/callbacks(与 LinearLeaf 同一套改造);补 sklearn wrapper 测试 + GAMClassifier

## 贯穿:信任基础设施 ✅ 全部完成(2026-07-20)

- [x] **GPU CI(nightly,Modal)**:跑 gpu/parity 标记套件;CRIT-5/6 能存活至今就是因为 CUDA 路径
      在 CI 中从未执行 — `tests/modal_gpu_tests.py` 已存在,接上即可
- [x] XGBoost 差分测试已有(test_numerical_agreement.py),扩展覆盖到 growth 策略接线后的 leafwise/symmetric
- [x] **文档-代码一致性检查**:本次审计发现 4 处文档示例照抄即崩(growth 参数、GAM max_depth、

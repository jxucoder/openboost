...
允许重写 API、trainer、内部表示和持久化格式**。规划与 F0.1 未启动重写或 GPU 作业；
后续按下面的依赖和验收顺序执行。

共同构成 v1 规格的文件：

- 本文件：产品目标、范围、架构、工作依赖、阶段交付与完成定义。
- [应用覆盖与案例契约](foundation-application-contracts.md)：全部必需用例 A1–A13 与数据选择。
- [F0.1 算法任务卡](foundation-tasks.md)：逐项输入/输出、数据、数学、判错、对照和接口草图。
- [Foundation 工程构建设计](foundation-construction-design.md)：模块、数据结构、函数契约、
  建树/状态/设备执行及 B01–B14 构建切片；后续实现的具体设计依据。
...
用户进一步明确：**列出的用例都需要，逐项交付和验收**。分类、回归、ranking、
分位数、多输出、计数、正值金额、总损失、生存、分布预测、Formula 和 train-many
共同约束 foundation；保险/AFT 没有特殊优先级。完整覆盖见 A1–A13，不能用少数
“重要案例”或一个总任务数代替。实现顺序由组件依赖决定，全部用例进入 v1 完成条件。
...
每个用例本身必须有实现、独立语义检查和真实任务评测；具体数据集可以选择，
不能据此把用例改成 optional。A1–A13 的逐项验收与 R1–R9/C1–C7 一起构成 v1 范围。
...
本节是架构总览；可执行到模块/函数的决定见 [工程构建设计](foundation-construction-design.md)。
规划交付包含“怎样构建 foundation”，任务卡和 eval 为该实现提供约束与判卷依据。
F0.2 的独立数学参考是准备工作，实际公共组件、recipe 与 runtime 在 F1 构建。
...
构建设计给出第一版模块与记录的具体选择，优先少量数组、结构化状态和普通函数。
这些选择可据 F1 的实际调用修订；由多个使用方检验后再冻结为公共协议。
...
- [x] **F0.1 — 任务卡与替代方案审计。** 已交付 [任务卡](foundation-tasks.md)：
  从四类结构探针出发，固定全部 required recipes 的输入/输出、两轮状态变化、
  支持边界和独立验收。
  明确 incumbent 的现成接口、可修改源码的路径及其成本；尤其纳入 Py-Boost。
  按 R1–R9 / C1–C7 / A1–A13 形成完整矩阵，逐项写清 release 已发布/计划中状态、
  支持边界、输入/目标/输出、独立参考、数据处理、质量指标、阶段和证据入口。
  每个 A 项都有任务卡；数据尚未选定则登记待完成，不能删除该用例或借另一项过关。
  交付任务卡 + 简短接口草图；每项需求能映射到 E0–E6，不再新增一套空泛路线图。
  规格覆盖检查通过；数据 bytes/hash、实际新版本 capability smoke、资源预算与保留题
...
- [ ] **F0.2 — 原始 NumPy 参考与反例。** 新建 `tests/v1/reference/`，
  ...
  覆盖各用例特有语义：class/link、query/pair、加权分位数、向量输出、offset/权重/
  预测单位、事件密度与删失概率、分布几何、formula Jacobian 和 run isolation。
  边界标签中的合法无穷大不能被通用 finite-vector 校验误拒绝。
...
F0 出口：全部 A1–A13 任务卡齐全、可判对错、比较对象公平、修改目标明确。
尚无数据或判卷标准的任务使 F0 未完成，不能靠满足一个总数量过关。
先完成 F0.1、F0.2、F0.3；设计和数学参考可分多个独立提交，禁止夹带模型迁移或 kernel 优化。
...
- [ ] **F1.5 — 各用例的数据/目标契约。** 接通 class mapping/link、query/pair、
  quantile residual/weight、vector output、Poisson/Gamma/Tweedie 的目标单位与 offset、
  AFT 的事件/删失项，以及分布/formula 的结构输入和参数输出。用 F0 的独立反例
  验证索引、接受状态、推理和新格式 round trip。若需在通用 runner 按任务名特判，
  先修正边界；各 recipe 按依赖分提交。AFT 未实现的删失类型仍须显式拒绝。
- [ ] **F1.6 — 完整 CPU 覆盖。** 补齐 binary/multiclass、group ranking、weighted
  ...
  A1–A13 各自关联可运行 CPU recipe/工作流，不允许只保留任务卡或示意 demo。
...
- [ ] 按 E3 逐项完成 A1–A13 的真实任务验收，全部用例都有原始 artifact，数据
  合计至少6个独立来源。Poisson、Gamma、Tweedie 分别评测；分布预测有 proper score，
  Formula 同时有结构反例与真实任务，train-many 有完整模型选择结果和成本。
  使用强 baseline、预注册质量指标、独立测试集、重复种子/折，记录全部失败。
  不能从矩阵中只挑若干代表任务，也不能以合成 Formula 代替真实任务交付。
...
每阶段 commit 附对应 E-gate、原始记录和未完成单元。F5 检查 A1–A13 逐项有证据，
且 E0–E6 全部 required 通过才称“工程 v1 完成”；E7 单列，决定能否声称外部
adoption/impact。任何用例未完成都保持未完成，不能换成“后续案例”后宣布 v1 完成。
每个大阶段末重看任务与 component 的复用关系，不仅检查完成了多少函数。

计划中的新增实现目录以公共模块/普通函数组织：data/targets、stats/ops、tree、
objectives、runtime、recipes、artifacts 和 models；具体职责/依赖见工程构建设计。
可替换现有私有模块，

...
OpenBoost is all-Python, so every extension point is a plain Python object
plus a registry call: no C++ plugins, no recompilation.

If the thing you want to boost is a **formula** `y = f(θ, x)`, you do not
need a registry: use [FormulaBoost](../user-guide/formulaboost.md)
and pass the callable. If it is a **distribution NLL**, use the distribution
registry below. If it is **right-censored Weibull**, use
[WeibullAFT](../user-guide/survival.md).

Three registries make custom components usable **by name**, exactly like
the built-ins, and one decorator opts a custom loss into GPU-native
execution:
...
- [FormulaBoost](../user-guide/formulaboost.md): boost parameters of a formula, no registry required.

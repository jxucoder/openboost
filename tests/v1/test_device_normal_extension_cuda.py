"""Installed external D2 on Normal CUDA; requires a separate fresh CPU interpreter."""
...
def test_installed_d2_normal_and_fresh_cpu(update, mode, damping, fixed, tmp_path):
    ...
    directory = Path(os.environ.get("OPENBOOST_NORMAL_ARTIFACTS", tmp_path)) / (
    ...
    for repeat in range(2):
        ...
        with ExecutionContext() as context:
            ...
            measurements.append(
                dict(
                    repeat=repeat,
                    fit_including_context_preparation_export_seconds=fit_seconds,

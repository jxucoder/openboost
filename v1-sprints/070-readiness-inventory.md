...
| Boundary | Observed state | Next falsifiable check |
| --- | --- | --- |
...
| Authoring accounting | No end-to-end independent dispatch/accounting runner was found in the inspected evaluation tooling | Make real generated-token counts, time states and budget stopping observable; unknown token usage cannot pass |
...
1. Audit remaining worker call paths and establish the actual candidate/judge data
   boundary using existing runners, without another universal execution framework.
2. Add enforced-environment fault probes for test-label/verifier access and exact
   worker resource policy; retain actual process errors separately from injected
   judge statuses. Freeze evaluator-owned expected matrices from the design.
3. Prepare 069's executable accounting packet. Record model/settings/tool/version
   and token-observation gaps explicitly; do not turn the designer's implementation
   time or synthetic token events into independent author-cost evidence.
4. Only then preflight the full A6/A13 search and decide whether expansion is justified.
